...
public class FLSMHosts {
	private IPAdress netAddress;
	private ArrayList<IPAdress> networkIPs;
	private byte newpfx;
	private short numberNetworks;
	private short numberHosts;
	private short neededHosts;
	//---------------------------------------------

	public FLSMHosts(short b1,short b2,short b3,short b4,byte pf,short n) {
	...
	public short getHosts() {
		return neededHosts;
	}

	public void setHosts(short hosts) {
		neededHosts = hosts;
	}
	//----------------------------------------------------------------
	public void subNetting() {
		byte bk=1;
		while(Math.pow(2,bk)-2<neededHosts) {
			bk++;
		}
		numberHosts=(short)((Math.pow(2,bk))-2);
		byte bw=(byte)((32-netAddress.getPrefix())-bk);
